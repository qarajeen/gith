            ...
            <Card className="w-full bg-card/80 backdrop-blur-sm border-white/10 relative">
                ...
                {(step > 1 || (step === 1 && formData.serviceType)) && (
                    <CardFooter className="flex items-center justify-between gap-4 p-6 sm:bg-transparent">
                        ...
                        <div className="fixed bottom-4 left-4 right-4 sm:static flex items-center justify-between sm:justify-end gap-4 w-full sm:w-auto z-20">
                             {step > 1 && (
                                <Button variant="outline" onClick={prevStep} size="lg" className="shadow-lg sm:shadow-none sm:hidden"><ArrowLeft className="mr-2 h-5 w-5"/> Back</Button>
                            )}
                            {step < 4 ? (
